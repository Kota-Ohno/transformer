...
- 隠れ層次元: 512
- アテンションヘッド数: 8
- エンコーダー/デコーダー層数: 4 (GPUメモリに応じて自動調整)
- Feed-forward次元: 2048
- ドロップアウト率: 0.1
- 相対位置エンコーディングの最大距離: 128

これらのパラメータは`config.py`で変更可能です。

### 重要な注意事項

学習時と推論時のモデル設定の一貫性が重要です。設定が一致しない場合、モデルの読み込みに失敗することがあります。特に以下の設定は重要です：

- **NUM_LAYERS**: エンコーダー/デコーダー層数
- **REL_POS_MAX_DISTANCE**: 相対位置エンコーディングの最大距離

GPUメモリに基づく自動調整機能により、学習時に層数が調整される場合があります。モデルロードエラーが発生した場合は、保存されたモデルの設定を確認してください。
...
バッチサイズだけでなく、GPUメモリ制約に基づいて自動的にモデルサイズも調整されることに注意してください。特に、エンコーダー/デコーダー層数は利用可能なGPUメモリに応じて以下のように決定されます：

- 16GB以上: 4レイヤー
- 8GB以上16GB未満: 4レイヤー
- 4GB以上8GB未満: 4レイヤー（hidden_size=384, heads=6）
- 4GB未満: 3レイヤー（hidden_size=256, heads=4）
