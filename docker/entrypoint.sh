...
if [ -z "$TRANSFORMER_GLOBAL_DEVICE" ] && [ -z "$TRANSFORMER_DEVICE" ]; then
    if ! python -c "import torch; exit(0 if torch.cuda.is_available() else 1)" 2>&1; then
        ...
        # まずデバイス数が0より大きいことを確認
        if ! python -c "import torch; assert torch.cuda.device_count() > 0" 2>&1; then
            echo "Warning: No CUDA devices found. Falling back to CPU."
            export TRANSFORMER_GLOBAL_DEVICE=cpu
            export TRANSFORMER_DEVICE=cpu
        else
            # 最初の利用可能なデバイスをプローブ
            device_index=0
            if ! python -c "import torch; torch.cuda.get_device_properties($device_index)" 2>&1; then
                echo "Warning: CUDA device access failed for device $device_index. Falling back to CPU."
                export TRANSFORMER_GLOBAL_DEVICE=cpu
                export TRANSFORMER_DEVICE=cpu
            else
